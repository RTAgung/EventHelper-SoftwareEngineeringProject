    ...
    <main class="container-fluid p-5 text-white bg-dark2" >
      <div class="row justify-content-md-center">
        <div class="col-6 bg-dark p-5">
          ...
          <?php
            $id = $_GET['id'];
            $sqlExpense = "SELECT * FROM detail_expenses WHERE Id = $id";
            $dataExpense = mysqli_query($link, $sqlExpense);
            $rowExpense = mysqli_fetch_object($dataExpense);
           ?>
          <form method="post" action="event_expenseeditprocess.php?id=<?=$id?>">
            <div class="form-group">
              <label>Object</label>
              <input type="text" class="form-control bg-transparent text-white rounded-0" name="object" value="<?=$rowExpense->Object?>" placeholder="Object" required>
            </div>
            <div class="form-group">
              <label>Quantity</label>
              <input type="number" class="form-control bg-transparent text-white rounded-0" name="quantity" value="<?=$rowExpense->Quantity?>" required>
            </div>
            <div class="form-group">
              <label>Price</label>
              <input type="number" class="form-control bg-transparent text-white rounded-0" name="price" value="<?=$rowExpense->Price?>" required>
            </div>
            <div class="form-group">
              <label>Information</label>
              <textarea class="form-control bg-transparent text-white rounded-0" name="information" rows="3"><?=$rowExpense->Information?></textarea>
            </div>
