    ...
    <main class="container-fluid p-5 text-white bg-dark2" >
      <div class="row justify-content-md-center">
        <div class="col-6 bg-dark p-5">
          <h1 class="text-center mb-4">EDIT INCOME</h1>
          <?php
            $id = $_GET['id'];
            $sqlIncome = "SELECT * FROM detail_incomes WHERE Id = $id";
            $dataIncome = mysqli_query($link, $sqlIncome);
            $rowIncome = mysqli_fetch_object($dataIncome);
           ?>
          <form method="post" action="event_incomeeditprocess.php?id=<?=$id?>">
            <div class="form-group">
              <label>Source</label>
              <input type="text" class="form-control bg-transparent text-white rounded-0" name="source" value="<?=$rowIncome->Source?>" placeholder="Source" required>
            </div>
            <div class="form-group">
              <label>Quantity</label>
              <input type="number" class="form-control bg-transparent text-white rounded-0" name="quantity" value="<?=$rowIncome->Quantity?>" required>
            </div>
            <div class="form-group">
              <label>Price</label>
              <input type="number" class="form-control bg-transparent text-white rounded-0" name="price" value="<?=$rowIncome->Price?>" required>
            </div>
            <div class="form-group">
              <label>Information</label>
              <textarea class="form-control bg-transparent text-white rounded-0" name="information" rows="3"><?=$rowIncome->Information?></textarea>
            </div>
