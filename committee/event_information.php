    ...
    <main class="container-fluid p-5 text-white bg-dark2" >
      <div class="row justify-content-md-center">
        <div class="col-5 bg-dark p-5">
          <h1 class="text-center mb-4">ADD INFORMATION</h1>
          <?php
            $sqlEvent = "SELECT * FROM detail_events WHERE EventId = $eventId";
            $dataEvent = mysqli_query($link, $sqlEvent);
            $rowEvent = mysqli_fetch_object($dataEvent);
           ?>
          <form method="post" action="event_informationprocess.php">
            <div class="form-group">
              <label>Information</label>
              <textarea class="form-control bg-transparent text-white rounded-0" id="exampleFormControlTextarea1" name="information" rows="3"><?=$rowEvent->Information?></textarea>
            </div>
