      ...
      <div class="row justify-content-md-center">
        <div class="col-5 bg-dark p-5">
          <?php 
            ...
            $from = $_GET['f'];
            $from2 = $_GET['f2'];
            $sqlCom = "SELECT * FROM detail_committees WHERE CommitteeId = $comId";
            $dataCom = mysqli_query($link, $sqlCom);
            $rowCom = mysqli_fetch_object($dataCom);
           
            if ($from == "a") {
              echo "<h1 class='text-center mb-3'>EDIT DATA</h1>";
            } else if ($from == "p") {
              echo "<h1 class='text-center mb-3'>EDIT INVITE</h1>";
            }
           ?>
          <form method="post" action="com_editprocess.php?id=<?=$comId?>&f=<?=$from?>&f2=<?=$from2?>">
            <div class="form-group">
              <label>Email</label>
              <input type="text" class="form-control bg-transparent text-white rounded-0" name="comemail" readonly value="<?=$comEmail?>">
            </div>
            <div class="form-group">
              <label>Departement</label>
              <select class="rounded-0 form-control bg-transparent text-white" name="departid" required>
                <?php 
                  $sqlDepart = "SELECT * FROM Departements WHERE NOT Id = 'KEPA'";
                  $dataDepart = mysqli_query($link, $sqlDepart);
                  $selected = "";
                  while ($rowDepart = mysqli_fetch_object($dataDepart)) {
                    if ($rowCom->DepartementId == $rowDepart->Id) {
                      $selected = "selected";
                    }
                    echo "<option class='text-dark' value='$rowDepart->Id' $selected>$rowDepart->Name</option>";
                    $selected = "";
                  }
                 ?>
              </select>
            ...
            <div class="form-group">
              <label>Role</label>
              <select class="rounded-0 form-control bg-transparent text-white" name="role" required>
                <?php 
                  $selected = "";
                  if ($rowCom->Role == "head") {
                    $selected = "selected";
                  }
                  echo "<option class='text-dark' value='head' $selected>Head</option>";
                  $selected = "";
                  if ($rowCom->Role == "staff") {
                    $selected = "selected";
                  }
                  echo "<option class='text-dark' value='staff' $selected>Staff</option>";
                  $selected = "";
                 ?>
              </select>
            </div>
            <?php 
              if ($from == "a") {
                ?>
                  <div class="form-group">
                    <label>Job <small class="text-warning">*not required</small></label>
                    <textarea class="form-control bg-transparent text-white rounded-0" id="exampleFormControlTextarea1" name="job" rows="2"><?=$rowCom->Job?></textarea>
                  </div>
                <?php
              }
             ?>
            <div class="form-group py-1">
              <button type="submit" class="mt-1 btn btn-success btn-block rounded-0" name="submit">Update</button>
            </div>
          </form>
        </div>
      </div>
