    ...
    <main class="container-fluid p-5 text-white bg-dark2" >
      <div class="row justify-content-md-center">
        <div class="col-7 bg-dark p-5">
          <h1 class="text-center mb-3">INVITE REQUEST</h1>
          <?php 
            $from = $_GET['f'];
            if (isset($_GET['msg'])) {
              if ($_GET['msg'] == 'fc') {
                ?>
                <label class="text-warning">Invite failed, this email has become a committee</label>
                <?php
              } else if ($_GET['msg'] == 'fu') {
                ?>
                <label class="text-warning">Invite failed, there are no users with this email</label>
                <?php
              }
            }
           ?>
          <form method="post" action="com_inviteprocess.php?f=<?=$from?>">
            <div class="form-group">
              <label>Email</label>
              <input type="text" class="form-control bg-transparent text-white rounded-0" name="comemail" required>
            </div>
            <label>Departement</label>
            <div class="form-row">
              <div class="form-group col-md-8">
                <select class="rounded-0 form-control bg-transparent text-white" name="departementid" required>
                  <?php 
              ...
              <div class="form-group col-md-4">
                <a class="btn btn-outline-light btn-block rounded-0" href="com_adddepartement.php?f=<?=$from?>" role="button">Add Departement</a>
              </div>
            </div>
            <div class="form-group">
              <label>Role</label>
              <?php 
               ...
               ?>
              <input type="text" class="form-control bg-transparent text-white rounded-0" name="comrole" value="<?=$role?>" readonly>
            </div>
