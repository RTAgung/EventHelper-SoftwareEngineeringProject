    ...
    <main class="container-fluid p-5 text-white bg-dark2" >
      <div class="row justify-content-md-center">
        <div class="col-7 bg-dark p-5">
          ...
          <?php
            $runId = $_GET['id'];
            $sqlRundown = "SELECT * FROM detail_rundown WHERE Id = $runId";
            $dataRundown = mysqli_query($link, $sqlRundown);
            $rowRundown = mysqli_fetch_object($dataRundown);
           ?>
          <form method="post" action="event_rundowneditprocess.php?id=<?=$runId?>">
            <div class="form-group">
              <label>Activity</label>
              <input type="text" class="form-control bg-transparent text-white rounded-0" name="activity" value="<?=$rowRundown->Activity?>" placeholder="Activity" required>
            </div>
            <div class="form-group">
              <label>Start Time</label>
              <input type="time" class="form-control bg-transparent text-white rounded-0" name="starttime" value="<?=$rowRundown->StartTime?>" required>
            </div>
            <div class="form-group">
              <label>End Time</label>
              <input type="time" class="form-control bg-transparent text-white rounded-0" name="endtime" value="<?=$rowRundown->EndTime?>" required>
            </div>
            <div class="form-group">
              <label>Performer</label>
              <input type="text" class="form-control bg-transparent text-white rounded-0" name="performer" value="<?=$rowRundown->Performer?>" placeholder="Performer">
            </div>
            <div class="form-group">
              <label>Information</label>
              <textarea class="form-control bg-transparent text-white rounded-0" name="information" rows="3"><?=$rowRundown->Information?></textarea>
            </div>
