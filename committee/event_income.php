          ...
          <div class="nav">
            ...
            <div class="collapse" id="collapseEvent" aria-labelledby="headingOne" data-parent="#sidenavAccordion">
              <nav class="sb-sidenav-menu-nested nav">
                ...
                <a class="nav-link active" href="event_income.php">Income</a>
    ...
    <div id="layoutSidenav_content">
      ...
      <main class="container-fluid p-4">
        ...
        <div class="col-12 border p-4">
          ...
          <?php 
            $sqlIncome = "SELECT * FROM detail_incomes WHERE EventId = $eventId";
            $dataIncome = mysqli_query($link, $sqlIncome);

            $sqlSum = "SELECT SUM(x) as Total FROM (SELECT (Quantity*Price) as x FROM detail_incomes) y";
            ...
            $sqlEvent = "UPDATE detail_events SET TotalIncome = $sum WHERE EventId = $eventId";
          ...
          <div class="row mb-3">
            ...
            <div class="col-2 offset-6">
              <a class="btn btn-success btn-block rounded-0" href="event_incomeadd.php" role="button">Add Income</a>
            </div>
          </div>
          <table class="table table-hover">
            ...
            <tbody>
              <?php 
                $count = 0;
                while ($rowIncome = mysqli_fetch_object($dataIncome)) {
                  $count++;
                  ?>
                    <tr >
                      <td><?=$count?></td>
                      <td class="text-truncate"><?=$rowIncome->Source?></td>
                      <td class="text-truncate"><?=$rowIncome->Quantity?></td>
                      <td class="text-truncate"><?=$rowIncome->Price?></td>
                      <td class="text-truncate"><?php echo $rowIncome->Quantity*$rowIncome->Price;?></td>
                      <td><?=$rowIncome->Information?></td>
                      <td class="">
                        <a href="event_incomedelete.php?id=<?=$rowIncome->Id?>" class="btn btn-sm btn-danger rounded-0 float-right mx-1">
                          <i class="fa fa-trash"></i>
                        </a>
                        <a href="event_incomeedit.php?id=<?=$rowIncome->Id?>" class="btn btn-sm btn-success rounded-0 float-right mx-1">
                          <i class="fa fa-cog"></i>
                        </a>
                      </td>
                    </tr>
