          ...
          <div class="nav">
            ...
            <div class="collapse" id="collapseEvent" aria-labelledby="headingOne" data-parent="#sidenavAccordion">
              <nav class="sb-sidenav-menu-nested nav">
                ...
                <a class="nav-link active" href="event_expense.php">Expense</a>
              </nav>
            </div>
          </div>
        </div>
    ...
    <div id="layoutSidenav_content">
      ...
      <main class="container-fluid p-4">
        ...
        <div class="col-12 border p-4">
          ...
          <?php 
            $sqlExpense = "SELECT * FROM detail_expenses WHERE EventId = $eventId";
            $dataExpense = mysqli_query($link, $sqlExpense);

            $sqlSum = "SELECT SUM(x) as Total FROM (SELECT (Quantity*Price) as x FROM detail_expenses) y";
            $dataSum = mysqli_query($link, $sqlSum);
            $rowSum = mysqli_fetch_object($dataSum);
            $sum = $rowSum->Total;
            if ($sum == '') {
              $sum = 0;
            }

            $sqlEvent = "UPDATE detail_events SET TotalExpense = $sum WHERE EventId = $eventId";
          ...
          <div class="row mb-3">
            ...
            <div class="col-2 offset-6">
              <a class="btn btn-success btn-block rounded-0" href="event_expenseadd.php" role="button">Add Expense</a>
            </div>
          </div>
          <table class="table table-hover">
            ...
            <tbody>
              <?php 
                $count = 0;
                while ($rowExpense = mysqli_fetch_object($dataExpense)) {
                  $count++;
                  ?>
                    <tr >
                      <td><?=$count?></td>
                      <td class="text-truncate"><?=$rowExpense->Object?></td>
                      <td class="text-truncate"><?=$rowExpense->Quantity?></td>
                      <td class="text-truncate"><?=$rowExpense->Price?></td>
                      <td class="text-truncate"><?php echo $rowExpense->Quantity*$rowExpense->Price;?></td>
                      <td><?=$rowExpense->Information?></td>
                      <td class="">
                        <a href="event_expensedelete.php?id=<?=$rowExpense->Id?>" class="btn btn-sm btn-danger rounded-0 float-right mx-1">
                          <i class="fa fa-trash"></i>
                        </a>
                        <a href="event_expenseedit.php?id=<?=$rowExpense->Id?>" class="btn btn-sm btn-success rounded-0 float-right mx-1">
                          <i class="fa fa-cog"></i>
                        </a>
                      </td>
                    </tr>
