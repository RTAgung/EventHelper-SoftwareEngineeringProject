          ...
          <div class="nav">
            ...
            <div class="collapse" id="collapseCommittee" aria-labelledby="headingOne" data-parent="#sidenavAccordion">
              <nav class="sb-sidenav-menu-nested nav">
                ...
                <a class="nav-link active" href="com_staff.php">Staff</a>
              </nav>
            </div>
    ...
    <div id="layoutSidenav_content">
      ...
      <main class="container-fluid p-4">
        ...
        <div class="col-12 border p-4">
          <div class="row justify-content-end">
            <div class="col-3">
              <?php 
               ...
               ?>
              <a class="btn btn-success btn-block rounded-0 <?=$disabled?>" href="com_invite.php?f=s" role="button">Invite Request</a>
            </div>
          </div>
          <h2>Accepted</h2>
          <hr>
          ...
          <?php 
            $sqlCommittee = "SELECT * FROM committees c JOIN detail_committees dc ON c.Id = dc.CommitteeId WHERE c.EventId = $eventId AND c.Status = 'accepted' AND dc.Role = 'staff'";
                    ...
                    <tr >
                      ...
                      <td class="p-0">
                        <?php 
                          if ($role != "staff") {
                            ?>
                            <a href="com_delete.php?id=<?=$rowCommittee->Id?>&f=s" class="btn btn-sm btn-danger rounded-0 float-right mx-1">
                              <i class="fa fa-trash"></i>
                            </a>
                            <a href="com_edit.php?id=<?=$rowCommittee->Id?>&em=<?=$rowCommittee->Email?>&f=a&f2=s" class="btn btn-sm btn-success rounded-0 float-right mx-1">
                              <i class="fa fa-cog"></i>
                            </a>
                            <?php 
                          }
                         ?>
                        <a href="com_sertificate.php?id=<?=$rowCommittee->Id?>&us=<?=$rowUser->Username?>&em=<?=$rowCommittee->Email?>&f=s" class="btn btn-sm btn-primary rounded-0 float-right mx-1">
          ...
          <?php 
            $sqlCommittee = "SELECT * FROM committees c JOIN detail_committees dc ON c.Id = dc.CommitteeId WHERE c.EventId = $eventId AND c.Status = 'rejected' AND dc.Role = 'staff'";
            $dataCommittee = mysqli_query($link, $sqlCommittee);
           ...
           ?>
           <?php 
            $sqlCommittee = "SELECT * FROM committees c JOIN detail_committees dc ON c.Id = dc.CommitteeId WHERE c.EventId = $eventId AND c.Status = 'pending' AND dc.Role = 'staff'";
            $dataCommittee = mysqli_query($link, $sqlCommittee);
                          ...
                          <td class="p-0">
                            <?php 
                              if ($role != "staff") {
                                ...
                                <a href="com_edit.php?id=<?=$rowCommittee->Id?>&em=<?=$rowCommittee->Email?>&f=p&f2=s" class="btn btn-sm btn-success rounded-0 float-right mx-1">
                                  <i class="fa fa-cog"></i>
                                </a>
                                <?php 
                              }
